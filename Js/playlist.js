...
function buscarYMostrarAlbum(idAlbum){
    ...
    fetch(url)
        ...
        .then(function (album) {
            let player = document.querySelector('.reproductoralbum');
            playlistWrapperAlbum.innerHTML += '<div>' + '<li>' + '<a href="detail2.html?type=album&id=' + album.id + '">' + album.title + '</a></li>' + '</div>' + '<div>' + '<iframe class = "reproductoralbum" src="'  + 'https://www.deezer.com/plugins/player?format=classic&autoplay=false&playlist=true&width=700&height=350&color=007FEB&layout=dark&size=medium&type=album&id=' + idAlbum + '&app_id=1' + '" frameborder="0"></iframe>' + '</div>'
